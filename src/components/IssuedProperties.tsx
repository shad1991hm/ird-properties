...
const IssuedProperties: React.FC = () => {
  ...
  const filteredProperties = issuedProperties
    .filter(property => {
      ...
      const matchesFilter = filterType === 'all' || 
                           (filterType === 'permanent' && property.isPermanent) ||
                           (filterType === 'temporary' && !property.isPermanent);
  ...
  const getStats = () => {
    return {
      ...
      permanent: issuedProperties.filter(p => p.isPermanent).length,
      temporary: issuedProperties.filter(p => !p.isPermanent).length,
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-4">
          ...
          <div className="flex items-center space-x-2 mt-3 sm:mt-0">
            <span className={`px-2 py-1 text-xs font-medium rounded-full ${
              property.isPermanent ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
            }`}>
              {property.isPermanent ? 'Permanent' : 'Temporary'}
            </span>
          </div>
        </div>
